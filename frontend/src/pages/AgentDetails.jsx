...
export default function AgentDetails() {
  ...
  return (
  <>
   <div className='pt-[90px] pb-[100px] dark:bg-black'>
    <Page></Page>
     <div className="mx-auto p-5">
      ...
      <div className="flex justify-around border-b pb-5 mb-6">
        {tabs.map((tab, i) => (
          <button key={i}
            ...
            className={`pb-1 border-b-2 font-['Poppins'] ${activeTab === i ? 'border-pink-900 dark:border-yellow-500 dark:text-yellow-500 font-semibold' : 'border-transparent dark:text-white'}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      {activeTab < 2 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 px-[150px] justify-items-center">
          {dummyProperties.slice(0, visible * 3).map((prop, i) => (
            <div key={i} className="w-[329px] cursor-pointer bg-white dark:bg-pink-900 rounded-[20px] shadow-md justify-around">
              <img src='/house-agent.png' alt={prop.address} className="rounded-[20px] w-[329px] h-[299px] object-cover mb-3" />
              <div className="flex justify-between pb-1 px-5 text-xs dark:text-white">
                  <p className="text-gray-500">{prop.location}</p>
                  <p>{prop.rating} ★</p>
              </div>
              <h4 className="font-medium text-lg pb-1 px-5 dark:text-white">{prop.address}</h4>
              <p className="text-sm pb-1 px-5 dark:text-white">{prop.specs}</p>
              <p className="text-lg font-medium px-5 pb-2 dark:text-white">{prop.price}</p>
            </div>
          ))}
        </div>
      ) : (
        <div className="overflow-x-auto">
            <table className="w-full table-auto border-separate border-spacing-y-2 mb-6">
                <thead className="bg-orange-100 dark:bg-gray-800 text-sm text-gray-700 dark:text-white">
                  <tr>
                ...
                {dummyTableData.slice(0, visible * 3).map((item, i) => (
                    <tr key={i} className="text-sm text-gray-800 dark:text-white shadow-sm rounded">
      ...
      <div className="text-center mt-4">
        <button onClick={() => setVisible(v => v + 1)} className="border px-6 py-2 rounded-full hover:bg-gray-100 transition dark:text-yellow-500 dark:border-yellow-500 dark:hover:bg-yellow-500 dark:hover:text-black">
          See more
        </button>
      </div>
    </div>
